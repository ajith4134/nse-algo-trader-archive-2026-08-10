...
| 7 | Backtesting & Paper Trading | **in progress** (2026-07-23): foundation built — NseMarketClock + MarketClockGatedDataSourceRouter (24/7 replay↔live), Rule-F verified on 1,650 real INFY bars & the real NSE calendar; paper engine loop CLOSED (L4->L5->L6->ledger over real replayed bars, Rule-F verified on 22 real INFY sessions); next: §9 prediction-tables lab + slippage model + DSR/CPCV gate — realistic options slippage (`../PLAN.md` §1.2), MarketClock/DataSourceRouter 24/7 replay (§1.4), **and the Prediction-Labeled Trade Tables lab (§9) + institution concepts (§10): WIN / deliberate-LOSS / UNCERTAIN tables, immutable PredictionRecords with kill-criteria + mechanism fields, calendar partition keys, Referee audits — the paper engine is an experiment allocator, not one portfolio** | — |
| 8 | Session / Square-off Management | **core built (v1)** (2026-07-23): square-off schedule + safe-ordered retry-to-flat executor (short covered before hedge sold, never a naked leg); Rule-F verified — 0 positions open across 22 real sessions, survives injected broker outage | `08_session_square_off.md` |
| 9 | Dashboard, Monitoring & Alerting | **foundation built (v1)** (2026-07-23): read-model + editable control config (paper capital / min-per-trade / segment+strategy on-off / mode) + HTML renderer; published browser-reachable dashboard (phone/laptop) with the 16-trunk tree; Rule-F verified from a real paper run | `09_dashboard.md` |
