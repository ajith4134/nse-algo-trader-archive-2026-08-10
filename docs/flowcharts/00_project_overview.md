...
| 10 | Memory & Reflection (Plan 2 — case-based memory, council-of-models, Deflated-Sharpe/CPCV promotion gate, adversarial-validation trip wire, explainable memory; **primary training signal = prediction-error streams from the §9 trade-tables lab: reason ledger, reflection diffs, adversarial arm, double-sided hypothesis tests; plus §10 institution features in order: assumption registry, opponent ledger (participant-wise OI), information diet, epidemiology->antibody automation**) | **FUNCTIONALLY COMPLETE + Rule-F verified (2026-07-24)**: swappable `ExperienceMemory` (SQLite v1); reflection panel; assumption tripwires + **antibody auto-veto**; shadow-arm recovery; **§10 institution features ALL done** — assumption registry, opponent ledger (participant-wise OI: divergence→bias, volume→conviction, multi-day FII trend), information diet (inert-learning alert), epidemiology→antibody; proper scoring (log/quadratic); Brier decomposition (explainable memory); SQLite multi-hop (Graphiti REJECTED, research/50 — outcome-clustering); mechanism recalibration (memory corrects §9 win-prob). Notes `10_memory_reflection.md`, research/43-52. **Open (DATA/MARKET-blocked only):** multi-regime queries (need regime variety); shadow-arm LIVE pass (needs open market) | `10_memory_reflection.md` |
